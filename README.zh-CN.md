...
项目实际开发周期超过两周。历史文件名中的 `day1`、`day6` 等是早期开发时建立的**不可变实验批次编号**，不代表真实自然日。为了保持文件引用、校验和与复现命令有效，这些历史名称不再批量修改；对外按照功能阶段理解项目：
...
7. **最终消融与停止决策**：完成有界视觉重排实验并冻结 MVP 边界。
...
1. **视觉证据定位仍然较粗。** 系统可以判断页面是否含有表格、图片或矢量图形，但还不能稳定判断其中哪个视觉区域对应问题所问的实体、指标和时间。使用这些通用信号的预注册重排实验在 63 个 R1 问题上没有带来 Complete Evidence Recall 净提升。
2. **多页证据仍可能召回不完整。** RRF 的 Complete Evidence Recall@10 为 77.41%，因此仍有部分问题会遗漏至少一个必要页面，例如第二个比较数值、单位、表头或距离较远的续页。
3. **OCR 和扁平化布局文本可能丢失结构关系。** 扫描、旋转、表格和图表页面可能保留文字，却丢失行列关系或标签与数值的对应关系。冻结验证中的一个失败案例虽然检索到了正确图表页，但把数值关联到了错误的扁平化标签。
4. **评测结果限于开发阶段。** 主要结果来自 `development_tune` 和明确标注的诊断实验。由于有界 R1 准入实验未通过，本版没有执行 calibration 和 locked test，因此不声称具备 held-out test 性能。

## 后续方向

V2 最值得研究的是 **question-conditioned visual localization（问题条件化视觉定位）**：根据问题，在已经检索出的页面中进一步选择具体表格、图表或局部区域，同时保留页码来源和明确拒答路径。

该方法应先在文档隔离的开发数据上预注册；检索器、路由规则、证据预算和可靠性阈值全部冻结后，再在 held-out 数据上执行一次评测。针对单题添加规则、对所有文档无约束运行视觉模型，以及看到结果后继续调整权重，都不属于这一方向。
