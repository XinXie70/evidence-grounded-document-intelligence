...
- 包含 **291 个确定性测试**，覆盖数据隔离、检索、证据封装、推理、引用、拒答、成本控制与失败恢复。
- Locked test 始终未参与方法开发；本文只报告开发阶段实验结果，不把调参结果包装成最终测试成绩。
...
flowchart LR
    ...
    G --> F
    F --> H[Evidence Package<br/>排序页面 + 引用信息]
    H --> I[有依据的事实提取]
    I --> J[本地比较 / 算术]
    J --> K[引用与证据支持检查]
    K --> L{证据是否充分且通过验证?}
...
所有主要数字均来自文档隔离的 `development_tune`，或明确标记的小规模诊断实验。
...
随后，我们在 63 个 R1 视觉问题上测试了通用表格、图片和矢量对象信号能否改进 RRF Top-10 内的页面排序。Complete Evidence Recall@3 和 @5 均没有净提升，因此按预注册规则放弃该方案。这个负结果说明未来 V2 需要的是“问题与视觉区域的语义匹配”，而不只是判断页面有没有图表。
...
- 方法开发只使用 `development_tune`；协议原本将 `development_calibration` 保留给阈值选择，
  但本版 MVP 因 R1 准入实验未通过而没有执行 calibration。
