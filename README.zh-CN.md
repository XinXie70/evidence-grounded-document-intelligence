...
- 包含 **376 个确定性测试**，覆盖数据隔离、检索、证据封装、推理、引用、拒答、成本控制与失败恢复。
...
8. **校准与最终冻结**：独立裁判、人工一致性审计、确定性选择性回答、费用分批控制，
   以及一次性 locked-test 协议。
...
随后，在查看评分前冻结了问题条件化视觉检索器：把同一组 RRF Top-10 页面渲染为图片，使用固定版本的现成 ColSmol-256M 编码器和 late interaction 重排，不做 DocScope 专项训练，也不添加题号规则。在完整 63 题 R1 tune 切片上，Complete Evidence Recall **Top-3 从 35/63 提升到 42/63**，**Top-5 从 44/63 提升到 50/63**。所有预注册准入条件均通过，因此保留该重排器并进入校准阶段。这仍是开发集结果，尚不能代表 held-out 性能。
...
4. **最终 held-out 性能仍未知。** 开发和校准已经完成，但一次性 locked test 尚未打开，因此目前不声称最终测试集性能。

## 后续方向

完整 V1 系统现已冻结。紧接着要做的是：在单独获得授权后只打开一次 locked test，随后计算 bootstrap 置信区间并如实报告全部失败，不根据测试结果继续调参。后续 V2 可以继续研究选中页面内部的区域级 grounding。针对单题添加规则、对所有文档无约束运行视觉模型，以及看到结果后继续调整权重，仍然不属于这一方向。
